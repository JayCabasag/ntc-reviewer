...
function ReviewerCard({ data }: ComponentProps) {
  const { label, id, description } = data;
  return (
    <div className="max-w-sm p-6 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700 w-full">
      <div className="flex items-center">
        ...
        <Link className="pl-[15px] " to={`reviewer/${id}`}>
          <h5 className="capitalize font-extrabold text-transparent text-md md:text-lg bg-clip-text bg-gradient-to-r from-primary to-pink-600">
            {label}
          </h5>
        </Link>
      </div>
      <div>
        <p className="mb-3 text-[14px] md:text-base text-gray-600 font-normal">
          {description}
        </p>
        <Link
          to={`reviewer/${id}`}
