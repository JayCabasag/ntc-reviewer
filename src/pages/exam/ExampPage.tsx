...
import {
  ...
  useRemoveAnswers,
  useScore,
} from "@/contexts/answer-context/context-hooks";
import { Question } from "@/utils/types";

const reviewerListings: typeof ReviewerList = ReviewerList.flatMap(
  (data) => data
);

export default function ExamPage() {
  const { reviewerId } = useParams();
  const reviewerData = reviewerListings.find(
    (data) => `${data.id}` === reviewerId
  );
        ...
        <div>
          {examStatus === "submitted" && remarks === "failed" && (
            <div
              ...
              role="alert"
            >
              <span className="font-bold">You failed!</span> score :{" "}
              {`${score}/100`}
            </div>
          )}
          {examStatus === "submitted" && remarks === "passed" && (
            <div
              className="p-4 mb-4 text-lg text-green-800 rounded-lg bg-green-50 dark:bg-gray-800 dark:text-green-400"
              role="alert"
            >
              <span className="font-bold">Congratulations!</span> score :{" "}
              {`${score}/100`}
            </div>
          )}
          {examStatus === "in-progress" && (
            <div
              className="p-4 mb-4 text-lg text-yellow-800 rounded-lg bg-yellow-50 dark:bg-gray-800 dark:text-yellow-300"
              role="alert"
            >
              <span className="font-bold">Warning!</span> Do not refresh page to
              prevent answer lose.
            </div>
          )}
          {examStatus !== "in-progress" && (
